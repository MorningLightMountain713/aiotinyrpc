    ...
    [
       ...
       "params": [42, 23], "id": 1}""",
        ...
        "subtrahend": 23}, "id": 4}""",
            {
                "method": "subtract",
                "kwargs": {"minuend": 42, "subtrahend": 23},
                "unique_id": 4,
            },
        ),
        (
        ...
        ("""{"jsonrpc": "2.0", "method": "foobar"}""", {"method": "foobar"}),
    ],
)
...
def test_parsing_invalid_arguments(prot):
    with pytest.raises(JSONRPCInvalidParamsError):
        prot.parse_request("""{"jsonrpc": "2.0", "method": "update", "params": 9}""")
...
@pytest.mark.parametrize(("data"), ["""{"jsonrpc": "2.0", "result": 19, "id": 9001}"""])
...
def test_batch_invalid_array(prot):
    assert isinstance(prot.parse_request("""[1]""")[0], JSONRPCInvalidRequestError)
...
def test_jsonrpc_spec_v2_example1(prot):
    ...
    assert _json_equal(
        """{"jsonrpc": "2.0", "result": 19, "id": 1}""", reply.serialize()
    )
    ...
    assert _json_equal(
        """{"jsonrpc": "2.0", "result": -19, "id": 2}""", reply.serialize()
    )
...
def test_jsonrpc_spec_v2_example2(prot):
    ...
    request = prot.create_request("subtract", kwargs={"subtrahend": 23, "minuend": 42})
    ...
    assert _json_equal(
        """{"jsonrpc": "2.0", "result": 19, "id": 3}""", reply.serialize()
    )
    ...
    assert _json_equal(
        """{"jsonrpc": "2.0", "result": -19, "id": 4}""", reply.serialize()
    )
...
def test_jsonrpc_spec_v2_example3(prot):
    ...
    assert _json_equal(
        """{"jsonrpc": "2.0", "method": "foobar"}""", request.serialize()
    )
...
def test_jsonrpc_spec_v2_example4(prot):
    ...
    assert _json_equal(
        """{"jsonrpc": "2.0", "method": "foobar", "id": "1"}""", request.serialize()
    )
...
def test_jsonrpc_spec_v2_example6(prot):
    try:
        prot.parse_request("""{"jsonrpc": "2.0", "method": 1, "params": "bar"}""")
        assert False  # parsing must fail
