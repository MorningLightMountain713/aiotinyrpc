...
@pytest.fixture(params=[(), (("foo", "bar"), ("x", None), ("y", 42)), (("q", 1),)])
...
def test_proxy_calls_correct_method(
    ...
    mock_client.call.assert_called_with(
        prefix + method_name, method_args, method_kwargs, one_way=one_way_setting
    )


def test_client_uses_correct_protocol(
    client, mock_protocol, method_name, method_args, method_kwargs, one_way_setting
):
...
def test_client_send_binary_message(
    ...
    req = Mock(RPCRequest)
    req.serialize.return_value = "unicode not acceptable"
